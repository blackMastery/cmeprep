...
function Checkbox({
  ...
  // A partial "select all" reads as unchecked if it draws a tick, so the
  // indeterminate state gets its own glyph.
  ...
  return (
    <CheckboxPrimitive.Root
      data-slot="checkbox"
      className={cn(
        // `data-[state=checked]`, not `data-checked`: Radix emits
        // data-state="checked|unchecked|indeterminate", and Tailwind compiles
        // the `data-checked:` shorthand to a bare [data-checked] selector that
        // nothing ever matches — so the filled state never rendered.
