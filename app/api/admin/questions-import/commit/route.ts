...
import { listTaxonomy, nextPosition } from "@/lib/admin/taxonomy";
...
export async function POST(request: Request) {
  ...
  // Case-insensitive resolution mirrors the core; 23505 means an exact-name
  // race with another admin — refetch and continue (subjects-actions idiom).
  const taxonomy = await listTaxonomy();
  const subjectIdByNorm = new Map(
    taxonomy.map((s) => [normalizeKey(s.name), s.id])
  );
  const topicIdByNorm = new Map(
    taxonomy.flatMap((s) =>
      s.topics.map((t) => [
        `${normalizeKey(s.name)}::${normalizeKey(t.name)}`,
        t.id,
      ])
    )
  );
  ...
  let subjectPositions = taxonomy.map((s) => ({ position: s.position }));

  for (const planned of analysis.creationPlan.subjects) {
    const norm = normalizeKey(planned.name);
    if (subjectIdByNorm.has(norm)) continue;

    const { data, error } = await admin
      .from("subjects")
      .insert({ name: planned.name, position: nextPosition(subjectPositions) })
      .select("id")
      .single();
    ...
    if (error?.code === UNIQUE_VIOLATION) {
      const { data: existing } = await admin
        ...
        .single();
      if (!existing) return fail(500, `Could not create subject "${planned.name}".`);
      subjectIdByNorm.set(norm, existing.id);
      continue;
    }
    if (error || !data) {
    ...
    subjectIdByNorm.set(norm, data.id);
    subjectPositions = [...subjectPositions, { position: subjectPositions.length }];
    createdSubjects.push(planned.name);
  ...
  for (const planned of analysis.creationPlan.topics) {
    const subjectNorm = normalizeKey(planned.subjectName);
    const key = `${subjectNorm}::${normalizeKey(planned.name)}`;
    if (topicIdByNorm.has(key)) continue;

    const subjectId = subjectIdByNorm.get(subjectNorm);
    if (!subjectId) {
      return fail(500, `Missing subject for topic "${planned.name}".`);
    }

    const siblings =
      taxonomy.find((s) => normalizeKey(s.name) === subjectNorm)?.topics ?? [];
    ...
    if (error?.code === UNIQUE_VIOLATION) {
      ...
      topicIdByNorm.set(key, existing.id);
      continue;
    }
    if (error || !data) {
  ...
  for (const row of analysis.validRows) {
    ...
    if (topicId === PLACEHOLDER_TOPIC_ID) {
      const key = `${normalizeKey(row.subjectName)}::${normalizeKey(row.topicName)}`;
      const resolved = topicIdByNorm.get(key);
      if (!resolved) {
